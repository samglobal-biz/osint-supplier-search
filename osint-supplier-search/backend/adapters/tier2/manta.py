...
class MantaAdapter(BaseAdapter):
    """Manta — US small business directory with 30M+ companies."""
    name = "manta"
    rate_limit_rpm = 8
    cache_ttl_hours = 48
    ...
    async def search(self, job_id, query, filters):
        ...
        try:
            ...
            html = await self._get(
                f"https://www.manta.com/mb_{encoded}/",
                headers=self._bh(),
            )
            results = self._parse(html, query)
        except Exception as e:
            logger.warning("Manta failed", error=str(e))
    ...
    def _parse(self, html, query):
        ...
        results = []
        cards = tree.css("article.search-result") or tree.css("div.result-item") or tree.css("li[class*='result']")
        for card in cards[:20]:
            name = self._t(card, ["h2 a", "h3 a", ".company-name a", "a[class*='name']"])
            if not name:
                continue
            address = self._t(card, [".address", ".location", "address"])
            ...
            results.append(self._make_candidate(
                source_url=link or f"https://www.manta.com/mb_{urllib.parse.quote_plus(query)}/",
                raw_name=name, raw_country="US", raw_address=address, raw_phone=phone, raw_description=description,
            ))
        logger.info("Manta results", count=len(results))
        return results
